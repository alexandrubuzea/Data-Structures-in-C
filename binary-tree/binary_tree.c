...
b_tree_insert(b_tree_t* b_tree, void* data)
{
    ...
    while (!q_is_empty(q)) {
        b_node_tmp = (b_node_t *)*((void **)q_front(q));
        if (!b_node_tmp->left) {
...
static int
__b_tree_height(b_node_t *root)
{
    if (!root)
        return 0;
    
    int left = __b_tree_height(root->left);
    int right = __b_tree_height(root->right);

    return max(left, right) + 1;
}

int
b_tree_height(b_tree_t *tree)
{
    return __b_tree_height(tree->root);
}

static b_node_t *
__b_tree_lca(b_node_t *root, int x, int y)
{
    if (!root)
        return NULL;
    ...
    b_node_t *left = __b_tree_lca(root->left, x, y);
    b_node_t *right = __b_tree_lca(root->right, x, y);
...
b_node_t *
b_tree_lca(b_tree_t *tree, int x, int y)
{
    return __b_tree_lca(tree->root, x, y);
}